from flask import Flask, jsonify, request
from flask_cors import CORS  # Import CORS
import pandas as pd
import joblib

app = Flask(__name__)
CORS(app)  # Enable CORS

# Load your model and data here
df = pd.read_csv('english_hindi_songs_with_features.csv')  # Update with your actual data file
model = joblib.load('song_recommendation_model.pkl')

# Function to assign mood with relaxed workout conditions
def assign_mood(row):
    if row['energy'] > 0.7 and row['valence'] > 0.7 and row['tempo'] > 100:
        return 'happy'
    elif row['energy'] < 0.3 and row['valence'] < 0.3 and row['tempo'] < 90:
        return 'sad'
    elif row['energy'] > 0.7 and row['valence'] > 0.5 and row['tempo'] > 120:
        return 'excited'
    elif 0.5 < row['energy'] <= 0.8 and 0.4 < row['valence'] <= 0.7 and (60 <= row['tempo'] <= 100):
        return 'romantic'
    elif row['energy'] > 0.6 and row['tempo'] > 120:
        return 'dancing'
    elif row['energy'] > 0.6 and row['tempo'] > 100:  # Relaxed conditions for workout
        return 'workout'
    else:
        return 'tired'


# Apply mood to the dataset
df['mood'] = df.apply(assign_mood, axis=1)

def recommend_songs_based_on_mood(mood_input, df):
    # Shuffle the dataset to ensure randomness each time
    df = df.sample(frac=1).reset_index(drop=True)
    
    # Filter the dataframe to find songs with the same mood
    filtered_songs = df[df['mood'] == mood_input]
    
    # Drop duplicate track names to ensure unique song recommendations
    filtered_songs = filtered_songs.drop_duplicates(subset=['track_name'])

    # Separate English and Hindi songs, then take the first 5 unique songs after shuffling
    english_songs = filtered_songs[filtered_songs['language'] == 'English'].head(5)
    hindi_songs = filtered_songs[filtered_songs['language'] == 'Hindi'].head(5)

    return {
        'english_songs': english_songs[['track_name', 'artist', 'spotify_uri']].to_dict(orient='records'),
        'hindi_songs': hindi_songs[['track_name', 'artist', 'spotify_uri']].to_dict(orient='records')
    }

@app.route('/recommend', methods=['GET'])
def recommend():
    mood_input = request.args.get('mood')
    recommendations = recommend_songs_based_on_mood(mood_input, df)

    return jsonify(recommendations)

if __name__ == '__main__':
    app.run(debug=True)
